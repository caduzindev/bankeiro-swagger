...
describe('glob Files matches method', () => {
  ...
  it('should return example.txt and excluded/example.txt file', done => {
    ...
    globFilesMatches(baseDir, filePath)
      .then(files => {
        const [exampleFile, excludedFile] = files.map(file => file.replace(/\\/g, '/'));
        expect(exampleFile.includes('example.txt') || excludedFile.includes('example.txt')).toBe(true);
        expect(excludedFile.includes('excluded/example.txt') || exampleFile.includes('excluded/example.txt')).toBe(true);
